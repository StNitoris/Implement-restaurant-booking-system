...
void handleClient(int clientFd, Restaurant &restaurant, std::mutex &mutex, const std::string &staticRoot) {
    ...
    HttpResponse response;
    if (request.method == "OPTIONS" && startsWith(request.path, "/api/")) {
        response.status = 204;
    ...
    if (startsWith(request.path, "/api/")) {
        response.headers.emplace_back("Access-Control-Allow-Origin", "*");
        response.headers.emplace_back("Access-Control-Allow-Methods", "GET,POST,PUT,DELETE,OPTIONS");
