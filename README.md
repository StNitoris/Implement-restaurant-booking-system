...
Web 版本会在本地启动一个极简的 HTTP 服务，默认挂载 `web/` 目录下的静态前端页面。启动后在浏览器访问 `http://localhost:8080` 即可看到完整界面：
...
> 小贴士：
> - 如果你直接从文件系统打开 `web/index.html`（`file://` 协议），前端会自动将所有 API 请求指向 `http://localhost:8080`，服务端也已加入 CORS 头用以放行跨域请求。请确保先启动 `restaurant_booking_server`，否则页面会提示无法连接。
> - 如需改用其他后端地址，可在浏览器控制台设置 `window.__BOOKING_API_BASE__ = "http://你的地址"` 后再刷新页面。
